...
class MultiScaleInputMultiOutput3DUNet(EncoderDecoderSkeleton):
    def __init__(self,
        ...
        #pdb.set_trace()
        if len(self.model_parallel) == 2:  # Split encoder and decoder on different GPUs
            '''
            # ================== Split to GPU 0 ==================
            encoder_modules = nn.ModuleList(
                [module.cuda(self.model_parallel[0]) for module in self.encoder_modules])
            self.encoder_modules = encoder_modules

            downsampling_modules = nn.ModuleList(
                [module.cuda(self.model_parallel[0]) for module in self.downsampling_modules])
            self.downsampling_modules = downsampling_modules

            self.base_module.cuda(self.model_parallel[0])

            #================== Split to GPU 1 ==================
            upsampling_modules = nn.ModuleList(
                [module.cuda(self.model_parallel[1]) for module in self.upsampling_modules])
            self.upsampling_modules = upsampling_modules
            print([param.get_device() for param in upsampling_modules.parameters()])

            decoder_modules = nn.ModuleList(
                [module.cuda(self.model_parallel[1]) for module in self.decoder_modules])
            self.decoder_modules = decoder_modules
            print([param.get_device() for param in decoder_modules.parameters()])

            merge_modules = nn.ModuleList(
                [module.cuda(self.model_parallel[1]) for module in self.merge_modules])
            self.merge_modules = merge_modules
            print([param.get_device() for param in merge_modules.parameters()])

            self.final_module.cuda(self.model_parallel[1])
            print([param.get_device() for param in self.final_module.parameters()])
            '''
            # ================== Split to GPU 0 ==================
            self.encoder_modules.cuda(self.model_parallel[0])
            self.downsampling_modules.cuda(self.model_parallel[0])
            self.base_module.cuda(self.model_parallel[0])

            # ================== Split to GPU 1 ==================
            self.upsampling_modules.cuda(self.model_parallel[1])
            self.decoder_modules.cuda(self.model_parallel[1])
            self.merge_modules.cuda(self.model_parallel[1])
            self.final_module.cuda(self.model_parallel[1])
            #self.skip_modules.cuda(self.model_parallel[0])
        ...
        if len(self.model_parallel) == 2:  # Split encoder and decoder on different GPUs
            self.output_branches.cuda(self.model_parallel[1])

        self.autopad_feature_maps = AutoPad() if number_multiscale_inputs > 1 else None
        if len(self.model_parallel) == 2:  # Split encoder and decoder on different GPUs
            self.autopad_feature_maps.cuda(self.model_parallel[0])

        self.properly_init_normalizations()

    def forward(self, *inputs):
        #list(map(print, [f'i.shape {i.shape}' for i in inputs]))
        ...
        #pdb.set_trace()
        if len(self.model_parallel) == 2:
            _encoded_states = []
            for state in encoded_states:
                _encoded_states.append(state.cuda(self.model_parallel[1]))
            encoded_states = _encoded_states
            current = current.cuda(self.model_parallel[1])

        #print([param.get_device() for param in self.upsampling_modules.parameters()])
        #pdb.set_trace()
        ...
        if len(self.model_parallel) == 2:
            ...
            for output in outputs:
                _outputs.append(output.cuda(self.model_parallel[0]))
            outputs = _outputs
        ...
        '''for idx, out_vol in enumerate(outputs):
            max_out, min_out = torch.max(out_vol), torch.min(out_vol)
            print(colored(f'model Depth: {idx}, in_range: {max_out, min_out}', 'blue', attrs=['bold']))'''
