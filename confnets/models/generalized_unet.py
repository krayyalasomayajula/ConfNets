...
from inf3dUnet.unet3d.buildingblocks import Encoder, DecoderOnly, DoubleConv
import pdb
    ...
    def construct_encoder_module(self, depth):
        ...
        if depth == 0:
            first_conv = ConvNormActivation(f_in, f_out, kernel_size=(1, 5, 5),
                                           ...
                                           normalization="GroupNorm")
            # Here the block has a different number of input channels:
...
class MultiScaleInputMultiOutputPlant3DUNet(MultiScaleInputMultiOutput3DUNet):
    def __init__(self,
                 depth,
                 in_channels,
                 encoder_fmaps,
        ...
        self.unet_type = 'plant_seg'
        self.nb_norm_groups0 = nb_norm_groups0

        super(MultiScaleInputMultiOutputPlant3DUNet, self).__init__(
            depth,
            in_channels,
            encoder_fmaps,
            output_branches_specs,
            decoder_fmaps=decoder_fmaps,
            number_multiscale_inputs=number_multiscale_inputs,
            scale_factor=scale_factor,
            res_blocks_specs=res_blocks_specs,
            res_blocks_specs_decoder=res_blocks_specs_decoder,
            upsampling_mode=upsampling_mode,
            decoder_crops=decoder_crops,
    ...
    def construct_base_module(self):
        ...
        # Build blocks:
        res_block = Encoder(f_in, f_out,
                            basic_module=self.basic_module,
                            conv_layer_order=self.layer_order,
                            conv_kernel_size=self.conv_kernel_size,
                            num_groups=self.nb_norm_groups,
    ...
    def construct_encoder_module(self, depth):
        ...
        nb_norm_groups = self.nb_norm_groups0 if depth == 1 else self.nb_norm_groups
        #pdb.set_trace()
        # Build blocks:
        res_block = Encoder(f_in, f_out,
                            apply_pooling=not(depth == 0),  # skip pooling in the first encoder
                            basic_module=self.basic_module,
                            conv_layer_order=self.layer_order,
                            conv_kernel_size=self.conv_kernel_size,
                            num_groups=nb_norm_groups,
                            padding=self.conv_padding)
        return res_block

    def construct_decoder_module(self, depth):
        if self.basic_module == DoubleConv:
            in_feature_num = self.decoder_fmaps[depth+1] + self.encoder_fmaps[depth]
        else:
            in_feature_num = self.decoder_fmaps[depth+1]

        out_feature_num = self.decoder_fmaps[depth]
        # Build blocks:
        res_block = DecoderOnly(in_feature_num, out_feature_num,
                            basic_module=self.basic_module,
                            conv_layer_order=self.layer_order,
                            conv_kernel_size=self.conv_kernel_size,
                            num_groups=self.nb_norm_groups,
    ...
    def construct_upsampling_module(self, depth):
        ...
        sampler = UpsampleAndCrop(scale_factor=scale_factor, mode=self.upsampling_mode,
                                  crop_slice=self.decoder_crops.get(depth+1, None), unet_type=self.unet_type)
        return sampler
    ...
    def construct_merge_module(self, depth):
        return MergeSkipConnAndAutoCrop(self.decoder_fmaps[depth], self.encoder_fmaps[depth],
                                        nb_norm_groups=self.nb_norm_groups, unet_type=self.unet_type)
    ...
    def construct_output_branch(self,
        ...
        if depth == 0:
            ...
            sampling = UpsampleAndCrop(scale_factor=scale_factor, mode=self.upsampling_mode,
                                      crop_slice=self.decoder_crops.get(depth + 1, None), unet_type=self.unet_type)

        final_conv = ConvNormActivation(self.decoder_fmaps[depth],
                                        out_channels=out_channels,
                                        kernel_size=1,
                                        dim=3,
                                        activation=activation,
        ...
        out_branch = nn.Sequential(sampling, final_conv)
        return out_branch

class MultiScaleInputPlant3DUNet(MultiScaleInputMultiOutputPlant3DUNet):
    def __init__(self,
                 depth,
                 in_channels,
                 encoder_fmaps,
        ...
        super(MultiScaleInputPlant3DUNet, self).__init__(
            depth,
            in_channels,
            encoder_fmaps,
            [output_branches_specs,],
...
class MergeSkipConnAndAutoCrop(nn.Module):
    ...
    def __init__(self, nb_prev_fmaps, nb_fmaps_skip_conn,
                 nb_norm_groups=16, unet_type=None):
        self.unet_type = unet_type
        super(MergeSkipConnAndAutoCrop, self).__init__()

        if unet_type == 'plant_seg':
            self.conv = Identity()
        else:
    ...
    def forward(self, tensor, skip_connection):
        ...
        if self.unet_type == 'plant_seg':
            output = torch.cat((skip_connection, tensor), dim=1)
        else:
            output = self.conv(skip_connection) + tensor
        return output
